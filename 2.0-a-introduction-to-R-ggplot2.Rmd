...
```{r,echo=FALSE}
ggplot() +
  geom_point(aes(hp, qsec, shape = factor(vs), size = mpg), mtcars, alpha = 0.5, col = 'red') +
...
```{r}
g<-ggplot(msleep,aes(sleep_total, log(brainwt), label = name, col=vore)) #make sure to add the label aes!
...
```
 ...
 a) First plot. A simple histogram, and we use the stats binning:
...
```{r, echo=F}
ggplot(mtcars, aes(hp)) +
  stat_bin(bins = 10, col="black", fill = 'pink')
```

c) Third plot is a stat = density plot.

```{r, echo=F}
ggplot(mtcars, aes(hp, col = factor(am))) +
