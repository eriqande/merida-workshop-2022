...
birdgrid <- read_csv("data/6.0/wiflcurrentRH.csv") %>% 
  filter(BIO4!="-9999") #load your  random points with assocaited env predictors 
...
# Here it is in a tidyverse compliant way, which also keeps us from
# printing 100K rows in the bookdown!  When the birdgrid is a tibble
# it has be to coerced into a regular old data frame for the predict
# function to work.
currentgrid <- bind_cols(
    birdgrid %>% select(X,Y),
    predict(
      wiflforest,
      birdgrid %>% select(all_of(predictors_uncor)) %>% as.data.frame()
    )
  )
...
#run your PCs on your top four variables
PCs <- currentgrid %>%
  select(-X, -Y) %>%
  prcomp() 
...
Let's make those colors in a unified, coherent tibble.  Basically,
we have that red is PC1 + PC2 (scaled so that the smallest
is 0 and the max is 255), green is -PC2 (scaled) and blue
is PC3 + PC2 - PC1 (scaled).  So, let's keep all of that together:
```{r}
wifl_points <- bind_cols(
  currentgrid %>% select(X, Y),
  as_tibble(PCs$x)
) %>%
  mutate(    # get the raw rbg variables
    r = PC1 + PC2,
    g = -PC2,
    b = PC3 + PC2 - PC1
  ) %>%  
  mutate( # then scale them
    across(
      .cols = c(r,g,b),
      .fns = function(x) (x-min(x)) / (max(x)-min(x)) * 255
    ),
    rgb_hex = rgb(r, g, b, max = 255)
  ) 
```
...
Here, Eric does a ggplot version of that, not bothering with the arrows and stuff.
```{r}
ggplot(wifl_points, aes(x = PC1, y = PC2, colour = rgb_hex)) +
  geom_point(shape = 15) +
...
Down here, Eric does a similar thing, but from an sf/tidyverse perspective.  Plotting points with
geom_sf kind of sucks so, we retain X and Y for that.
```{r}
wifl_sf <- wifl_points %>%
  st_as_sf(
    coords = c("X", "Y"),
...
ggplot() +
  geom_point(
    data = wifl_sf, 
    mapping = aes(x = X, y = Y, colour = rgb_hex),
    shape = 15) +
  scale_colour_identity() +
  theme_void()
