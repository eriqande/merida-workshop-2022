...
And we might have genomic information to create a spatially explicit map of population structure, called the genoscape of the Willow Flycatcher.  
...
```{r, echo=FALSE, out.width='90%'}
knitr::include_graphics("images/6.0/GEA.Fig1.jpg")
```
...
Note, this is *not* to investigate or identify candidate genes, but rather what combination of environmental variables shapes overall population genetic structure of a species.
...
You can apply random forests with genomics. Brieuc et al. 2018 used this approach to rank the importance of SNPs for predicting a specific binary phenotype, *e.g.* disease resistance in fish. This is an example of one random tree, which can ultimately create a predictive polygenic model.

```{r, echo=FALSE, out.width='100%'}
knitr::include_graphics("images/6.0/RandomForest.Genomics.jpg")
```

Another interesting exmaple used random forests to find the predictive variants for cold hardiness, which was subsequently used to predict cold injury in the Sitka Spruce (Holliday et al. 2012).
...
a) reference SNPs,  
b) GI5 candidate gene,

you can see that different environmental variables predict genetic variation of the subsets and there are locally adpative pockets for each- different colors = locally adapted genotypes.
