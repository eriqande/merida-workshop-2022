...
The way this is typically done is to create a raster that has the
shape of the state of Yucatán polygon, and then using that state of
Yucatán raster as a _mask_ for the BioClim rasters.


In the variable
`yuc_bioc`, we have that BioClim raster data.  Let's remind ourselves of that:
```{r}
yuc_bioc
```
Note that the coordinate reference system is WGS 84.  

We also have the polygon data for the state of Yucatán:
```{r}
yuc_boundary
```

Note that the Coordinate Reference system (CRS) with the above
object is listed as unknown, though the datum used for it is WGS 84:
```{r}
st_crs(yuc_boundary)
```
### Explicitly setting the CRS on `yuc_boundary`

So, let's go ahead and set the CRS to WGS 84, just so that the raster and the
polygon are explicitly on the same CRS.  

The WGS84 CRS is known in the EPSG data base as 4326. So we will set that:
```{r}
st_crs(yuc_boundary) <- 4326
```

Now, note that the CRS is listed as WGS 84:
```{r}
yuc_boundary
```


### Creating a raster of the Yucatán polygon

For this we use the `rasterize()` function from the 'terra' package.
This function takes as its first argument a `SpatVector` object that gives the
shape to rasterize.  Its second argument is a `SpatRaster` object that tells
the function what extent and resolution the resulting raster should have.

### What is a `SpatVector`?

A `SpatVector` is a `terra` data type for holding vector spatial data.

Fortunately, it is easy to convert as `sf` object (like `yuc_boundary`) to
a `SpatVector` using the `vect()` function from the 'terra' package, like this:
```{r}
yuc_bound_spat_vect <- vect(yuc_boundary)

# see what that looks like when we print it:
yuc_bound_spat_vect
```
### Using the `rasterize()` function

Now, creating a raster from the Yucatán state polygon happens like this:
```{r}
yuc_bound_raster <- rasterize(
  x = yuc_bound_spat_vect,
  y = yuc_bioc
)
```

Plotting this, we see that everywhere within the shape of the state of Yucatán,
that cells have the value of 1, and everywhere outside the cells have the
value of NA:
```{r}
plot(yuc_bound_raster)
```

### Masking the BioClim rasters with the Yucatán state raster

Now that we have `yuc_bound_raster` we can use it to mask
the `yuc_bioc` data to only inlude cells within the state
boundaries.  This used the 'terra' function `mask()`, which is
quite simple.  
```{r}
yuc_state_bioc <- mask(yuc_bioc, yuc_spat_vect)
```
